...
Por ello nuestro modelo de datos estará modelado como una serie temporal con foco en la busqueda. La inserción de datos se realizará muy poco frecuentemente, idealmente una vez
...
\section{Resultados finales: Arquitectura de la sonda}
...
\subsubsection{Recolectando trazas}

% rsync a JIERRO contra sondas, limitado en bandwidth a X MiB

\subsubsection{Procesando trazas de containers \emph{SSH}}

% esto es un marron increible y tal como se hace ahora es lo siguiente:
% se lee el fichero de captura usando sysdig y mas filtros, y se saca la salida del proceso SSH.
% se agrupa por hilos de CPU y hay se hacen greps a CASCOPORRO.

% esto es una guarrada y habria que procesar el fichero binario, pero se ha de decir
% que SSH tiene secreto perfecto y es la unica manera de obtener información sin meternos en
% medio del protocolo.

% hay que explicar esto muy bien que sino no vale de nada este puto proyecto.

% guardamos procesado en dos indice de ES, por mes, con alias. Hay que explicar esto bien sino no vale de nada
% este puto proyecto.
...
% si añadimos containers de otros protocolos habra que añadir procesado.
