...
\begin{enumerate}
    ...
    \item[\emph{tracepoints}] A diferencia de un \emph{kprobe} o \emph{uprobe}, un tracepoint se define en el código fuente y se genera en tiempo de compilación, pudiendo ser activados en tiempo de ejecución cuando se requiera y extraer la información de ese \emph{tracepoint}.
...
el kernel de \emph{Linux} es modular, y por tanto se puede inyectar un módulo de código que extienda las capacidades del kernel, es la manera
...
de infrastructura como código (véase \cite{fowler-infra-as-code}). Dicha practica se basa en:
